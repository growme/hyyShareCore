...
import com.ccnet.core.common.utils.html.UserAgent;
import com.ccnet.core.common.utils.html.UserAgentUtil;
...
public class DeviceUtils {
	...
	public static boolean isMobileDevice(HttpServletRequest request) {
		...
		CPSUtil.xprint("当前是否为移动设备访问:"+isMobile);
		CPSUtil.xprint("当前是否为微信访问:"+isWeChat(request));
		CPSUtil.xprint("当前是否为IOS访问:"+isIOSDevice(request));
		return isMobile;
	}
	 ...
	 * @return
	 */
	public static String getDevDetailInfo(HttpServletRequest request) {
		String deviceDetail = "";
		if(request!=null){
			if(isMobileDevice(request)){//移动设备访问
				if(isWeChat(request)){
					deviceDetail = "浏览器:微信内置浏览器";
				}else{
					if(isIOSDevice(request)){
					   deviceDetail = "浏览器:iPhone|iPad|iPod设备内置浏览器";
					}else{
				     deviceDetail = "浏览器:安卓设备内置浏览器";
					}
				}
			}else{//pc端访问
				String userAgent = request.getHeader("user-agent");
				UserAgent client = UserAgentUtil.getUserAgent(userAgent);
				if(CPSUtil.isNotEmpty(client)){
					if(CPSUtil.isNotEmpty(client.getBrowserType())){
						deviceDetail+="浏览器："+client.getBrowserType();
					}
					if(CPSUtil.isNotEmpty(client.getBrowserVersion())){
						deviceDetail+=" 版本："+client.getBrowserVersion();
					}
					if(CPSUtil.isNotEmpty(client.getPlatformType())){
						deviceDetail+=" 操作系统："+client.getPlatformType();
					}
					if(CPSUtil.isNotEmpty(client.getPlatformSeries())){
						deviceDetail+=" "+client.getPlatformSeries();
					}
				}
			}
		}
		CPSUtil.xprint("deviceDetail="+deviceDetail);
		return deviceDetail;
	}
	...
	public static boolean isIOSDevice(HttpServletRequest request) {
		...
		String userAgent = request.getHeader("user-agent");
		for (int i = 0; !isMobile && userAgent != null
				&& !userAgent.trim().equals("") && i < ios_sys.length; i++) {
	 ...
	 * @return
	 */
	public static String getMobileName(String userAgent) {
		String model = "";
		if(CPSUtil.isNotEmpty(userAgent)){
		...
		//CPSUtil.xprint("通过userAgent解析出机型：" + model);  
		return model;
	}
	...
	public static void main(String[] args) {
		...
		String userAgent = "Mozilla/5.0 (Linux; Android 5.0.2; vivo Y51 Build/LRX22G; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/57.0.2987.132 MQQBrowser/6.2 TBS/044203 Mobile Safari/537.36 MicroMessenger/6.6.7.1321(0x26060739) NetType/4G Language/zh_CN";
		getMobileName(userAgent);
	}
