...
public class FileUtil {
	...
	public static void copyFile(String oldPathFile, String newPathFile) {
		try {
			...
			if (oldfile.exists()) { // 文件存在时
				// 新建文件输入流并对它进行缓冲   
		        FileInputStream input = new FileInputStream(oldPathFile);  
		        BufferedInputStream inBuff=new BufferedInputStream(input);  
		        
		        //处理文件夹生成
		        String newFolder = newPathFile.substring(0,newPathFile.lastIndexOf("/")+1);
		        File nfolder = new File(newFolder);
		        if(!nfolder.exists()){
		        	nfolder.mkdir();
		        }
	 ...
	 * @return
	 */
	public static boolean deletePicFile(String savePath){
		boolean temp = false;
		File diskFile = new File(savePath);
		if(diskFile.exists()){
			temp = FileUtil.delete(diskFile);
			if(temp){
				CPSUtil.xprint("删除图片【"+diskFile.getAbsolutePath()+"】成功了....");
			}else{
				CPSUtil.xprint("删除图片【"+diskFile.getAbsolutePath()+"】出错了....");
			}
		}
		return temp;
	}
