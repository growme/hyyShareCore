...
public class SpecCaptcha extends Captcha {
	...
	public String out(OutputStream out) {
		//处理字符串
		char [] strs = alphas();
		String codeString = String.valueOf(strs).toUpperCase();
		CPSUtil.xprint("pngcode="+codeString);
	...
	private boolean graphicsImage(char[] strs, OutputStream out) {
		...
		try {
			...
			for (int i = 0; i < 15; i++) {
				color = color(150, 250);
				g.setColor(color);
				g.drawOval(Randoms.num(width), Randoms.num(height), 5 + Randoms
						.num(10), 5 + Randoms.num(10));// 画蛋蛋，有蛋的生活才精彩
				color = null;
			}
			g.setFont(font);
			...
			for (int i = 0; i < len; i++) {
				...
				g.setColor(color);
				g.drawString((strs[i] + "").toUpperCase(), (width - (len - i) * w) + size,
						h - 4);
				color = null;
				ac3 = null;
			}
