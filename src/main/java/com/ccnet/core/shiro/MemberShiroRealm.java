...
public class MemberShiroRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
		 ...
		 MemberInfo a = memberInfoService.findFormatByLoginName(username);//通过登录名 寻找用户
	     if (a != null) {
	    	 if(UserSateType.VALID.getType().equals(a.getUserState())){
		    	//组合username,两次迭代，对密码进行加密 
		    	 String pwdEncrypt = CipherUtil.createPwdEncrypt(username,password,a.getSalt()); 
		    	 if(a.getLoginPassword().equals(pwdEncrypt)){
		    		 authenticationInfo = new SimpleAuthenticationInfo(a.getLoginAccount(), password, getName());
		    		 this.setSession(Const.MSESSION_USER, a);
